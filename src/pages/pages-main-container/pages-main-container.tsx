...
function PagesMainContainer(): JSX.Element {
  ...
  const originalOffers = useAppSelector(getOriginalOffers);
  ...
  const newOffers = useMemo(
    () => offers?.filter((item: TypeOffer) => item.city.name === activeCity),
    [activeCity, offers]
  );
  ...
  const onChange = useCallback(
    (type: TypeSorting) =>
      dispatch(
        setOffers(
          sortingPoint[type](type === 'Popular' ? originalOffers : newOffers)
        )
      ),
    [dispatch, newOffers, originalOffers]
  );

  const onListItemHover = useCallback(
    (id: string) => dispatch(fetchOffer(id)),
    [dispatch]
  );
    ...
    <div className="page page--gray page--main">
      ...
      <main
        className={`page__main page__main--index ${
          newOffers.length === 0 ? 'page__main--index-empty' : ''
        }`}
      >
                ...
                <li key={elem} className="locations__item">
                  <a
                    className={`locations__item-link tabs__item ${
                      elem === (activeCity as typeof elem)
                        ? 'tabs__item--active'
                        : ''
                    }`}
                    onClick={() => {
